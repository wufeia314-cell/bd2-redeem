...
def main():
    ...
    ap.add_argument("--nickname", dest="nickname", help="单个游戏昵称（即官方兑换身份）")
    ap.add_argument("--nickfile", dest="nickfile", help="昵称列表文件（每行一个）")
    ...
    nicks = []
    if args.nickname:
        nicks.append(args.nickname.strip())
    if args.nickfile:
        with open(args.nickfile, encoding="utf-8") as f:
            nicks += [ln.strip() for ln in f if ln.strip()]
    if not nicks:
        ap.error("请用 --nickname 或 --nickfile 指定游戏昵称")
